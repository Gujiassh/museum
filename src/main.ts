...
import { resourceLoader, type LoadProgress } from "./libs/ResourceLoader";
  ...
  buttonFactory.setToggleCallback(() => {
    ...
    if (buttonFactory.isCardVisible) {
      ...
      moveCameraAndRotateWithTargetAndZoom(
        ...
        config.cameraRotation.x, config.cameraRotation.y, config.cameraRotation.z,
  ...
  try {
    ...
    requestAnimationFrame(() => {
      ...
      buttonsConfig.forEach((config, index) => {
        ...
        buttonLabel.position.set(config.position.x, config.position.y, config.position.z);
...
// 获取当前相机状态的方法
function getCurrentCameraConfig(buttonId: string, buttonText: string, targetX: number, targetY: number, targetZ: number) {
  const config = {
    id: buttonId,
    position: {
      x: targetX,
...
function moveCameraAndRotateWithTarget(
  posX: number, posY: number, posZ: number,
  rotX: number, rotY: number, rotZ: number,
  targetX: number, targetY: number, targetZ: number
) {
...
function moveCameraAndLookAt(
  posX: number, posY: number, posZ: number,
  lookX: number, lookY: number, lookZ: number
) {
  console.log('移动相机到:', { x: posX, y: posY, z: posZ });
  console.log('朝向目标:', { x: lookX, y: lookY, z: lookZ });
  ...
  gsap.to(camera.position, {
    ...
    onUpdate: () => {
      // 在移动过程中实时更新朝向
      const targetPosition = new THREE.Vector3(lookX, lookY, lookZ);
...
function lookAtPosition(x: number, y: number, z: number) {
  // 创建目标位置向量
